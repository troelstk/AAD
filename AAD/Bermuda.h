    ...
    for(int i = 0; i<nPaths; ++i){
        ...
        for(int t = 1; t<exTimes.size(); ++t ){
           ...
           double EY2 = t < exTimes.size() - 1 ?
            ...
            double eta2 = EY2 < swap_val ? 1.0 : 0.0; // 1 if exercise
            
            swap_path += eta * swap_val * eta2;
...
/* Price a Bermuda Swaption using classical LSMC */
template<class T> T LMM_BermudaSwaptionLSMC(vector<vector<T>> & vol, vector<vector<T>> & corr,
                             vector<T> & initF, vector<double> & exTimes,
                             double t, double Ta, double Tb, T r_fix, double notional,
                             int seed1, int seed2, int nPaths, int nPaths_presim, int nSteps_y, double yearly_payments, int dim_n)
{
    ...
    for(int t = int(exTimes.size() - 2); t >= 1; --t )
    {
        ...
        for(int i = 0; i<indices.size(); ++i){
            //print("EY ", EY(i), " swap val ", swap_vals2[indices[i]][t] );
            if(swap_vals2[indices[i]][t] > EY(i) ) { // If ITM
    ...
    //print(payoff);
    return sum(payoff)/double(nPaths) ;
}
