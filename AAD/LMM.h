...
/*template<class T> T LMM_BermudaSwaption2(vector<vector<T>> & vol, vector<vector<T>> & corr,
                             vector<T> & initF, vector<T> & exTimes,
                             T t, T Ta, T Tb, T r_fix, T notional,
                             int seed1, int nPaths, int nPaths_presim, int nSteps_y, T yearly_payments, int dim_n)
{
    ...
    for(int t = 1; t<exTimes.size(); ++t ){
        ...
        for(int i = 0; i<nPaths_presim; ++i){
           if(swap_vals[i][t] > 0) { // If ITM
               ...
               ITMLibor.push_back(Libor[i][t]);
               indices.push_back(i);
               ITMY.push_back(swap_vals[i][t]); // WRONG, need payoff from next period
           };
        }
        size_t ITMsize = ITMswapRate.size();
        
        // Construct and fill out X matrix
        mat X(ITMsize, no_basis_funcs, fill::zeros);
        for(int i = 0; i<ITMsize; ++i){
           X(i, 0) = 1;
           X(i, 1) = ITMswapRate[i];
        ...
        beta.col(t-1) = V * D.t() * Sig * U.t() * vec(ITMY);
        print("beta col ", t-1, " is ", beta.col(t-1));
    } // Exercise dates-loop
    ...
    vec payoff(nPaths);
    for(int i = 0; i<nPaths; ++i){
        payoff(i) = swap_vals2[i][exTimes.size() - 1];
    }
    //print(payoff);
    ...
    for(int t = int(exTimes.size() - 2); t >= 1; --t ){
        ...
        for(int i = 0; i<ITMsize; ++i){
            if( swap_vals[indices[i]][t] > EY[i] ){
                payoff(indices[i]) = swap_vals2[indices[i]][t] ;
                print("payoff set to ", payoff(indices[i]), " at index ", i, " ", indices[i]);
            }
        }
    } // Exercise times
